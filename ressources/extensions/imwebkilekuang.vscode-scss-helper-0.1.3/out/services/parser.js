...
function getMethodParameters(ast, offset) {
    ...
    const paramsChildren = params.getChildren();
    const eachOffset = paramsChildren.map(item => ({ offset: item.offset, length: item.length }));
    if (node.getDeclarations) {
        eachOffset.push({
            offset: node.getDeclarations().offset,
            length: 0
        });
    }
    ...
    return paramsChildren
        ...
        let commentStr = null;
        if (node.getDeclarations) {
            const { offset: childOffset, length } = child;
            const nextOffset = eachOffset[index + 1];
            const commentOrigin = nodeText.slice(childOffset - offset + length, nextOffset.offset - offset);
            const commentArr = commentOrigin.match(/(\/\/\!\@)(.+)/);
            commentStr = (commentArr && commentArr.length) ? `[${commentArr[2].trim()}]` : null;
        }
        const value = defaultValueNode === undefined ? null : defaultValueNode.getText();
        return {
            name: child.getName(),
            offset: child.offset,
            value: value !== null && value !== void 0 ? value : commentStr
        };
    });
}
